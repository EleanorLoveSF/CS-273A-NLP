...
def load_unlabel_data(unlabel_dir_path):
    unlabel_file_paths = __get_file_path_list(os.path.join(unlabel_dir_path, 'unsup/'))
    unlabel_texts = __load_texts(unlabel_file_paths)
    test_text_list = list()
    test_text_list.extend(unlabel_texts)
    zipped_test_list = list(zip(test_text_list))
    random.shuffle(zipped_test_list)
    test_texts = zip(*zipped_test_list)
    return [*test_texts]


def load_vocab_dict(vocab_file_path):
    vocab_dict = dict()
    with open(vocab_file_path, 'r', encoding='UTF-8') as fp:
        for line in fp:
